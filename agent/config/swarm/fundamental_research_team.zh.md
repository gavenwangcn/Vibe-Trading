...
以下技能来自 `fundamental_research_team.yaml` 中各代理的 `skills` 字段，运行时由代理通过 `load_skill()` 按需加载。

| 代理 ID | 绑定的 Skill 技能 |
| --- | --- |
| `financial_analyst` | `financial-statement`、`fundamental-filter` |
| `valuation_analyst` | `valuation-model`、`earnings-forecast` |
| `quality_analyst` | `fundamental-filter`、`web-reader` |
| `report_editor` | `report-generate` |

**本工作流涉及的全部 Skill（去重，按字母序）：** `earnings-forecast`、`financial-statement`、`fundamental-filter`、`report-generate`、`valuation-model`、`web-reader`
