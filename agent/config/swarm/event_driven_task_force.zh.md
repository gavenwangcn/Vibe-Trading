...
以下技能来自 `event_driven_task_force.yaml` 中各代理的 `skills` 字段，运行时由代理通过 `load_skill()` 按需加载。

| 代理 ID | 绑定的 Skill 技能 |
| --- | --- |
| `event_scanner` | `event-driven`、`corporate-events`、`web-reader`、`geopolitical-risk` |
| `impact_analyst` | `corporate-events`、`sentiment-analysis`、`valuation-model` |
| `strategy_builder` | `event-driven`、`strategy-generate` |

**本工作流涉及的全部 Skill（去重，按字母序）：** `corporate-events`、`event-driven`、`geopolitical-risk`、`sentiment-analysis`、`strategy-generate`、`valuation-model`、`web-reader`
