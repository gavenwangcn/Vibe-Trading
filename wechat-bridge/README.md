...
微信消息**没有** Markdown / HTML 渲染，表格会变成纯文本。桥接会：

- 在 **`stripMarkdown`** 中把 **Markdown 表格**的每一格拆成**单独一行**并加 **`·`**，避免多列挤在同一行。
- 使用 **`toWeChatPlainText`** 对仍像「多列压一行」的长行做二次拆分（双空格分隔视为列）。

**仍建议**在 Agent / 技能说明里要求：面向微信时**少用表格**、多用**短段落 + 空行**、列表用 **`·` 或数字编号**，复杂数据可拆成「字段名：值」多行键值对。
